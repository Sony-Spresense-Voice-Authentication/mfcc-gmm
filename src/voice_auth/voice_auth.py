...
def get_voice_features(rate, data):
...
def build_model(name, paths):
    ...
    for path in paths:
        ...
        voice_features = get_voice_features(sampling_rate, data)
        if combined_features.size == 0:
            combined_features = voice_features
        else:
            combined_features = np.vstack([combined_features, voice_features])
...
def compare(path):
    ...
    for path in model_paths:
        ...
        features = get_voice_features(sampling_rate, data)
